...
file = open('text_recognize/label_map_cej.pickle', 'rb+')
...
def load_model(model_name):
    ...
    HN_gru = GRU(256, return_sequences=True,
                 kernel_initializer='he_normal', name='HN_gru', dropout=0.5)(HN_inner)
    HN_gru_b = GRU(256, return_sequences=True,
                   go_backwards=True, kernel_initializer='he_normal',
                   name='HN_gru_b', dropout=0.5)(HN_inner)
    ...
    VN_gru = GRU(256, return_sequences=True,
                 kernel_initializer='he_normal', name='VN_gru', dropout=0.5)(VN_inner)
    VN_gru_b = GRU(256, return_sequences=True,
                   go_backwards=True, kernel_initializer='he_normal',
                   name='VN_gru_b', dropout=0.5)(VN_inner)
    ...
    decoder_gru = GRU(256, return_sequences=True,
                      kernel_initializer='he_normal', name='decoder_gru', dropout=0.5)(res)
    decoder_gru_b = GRU(256, return_sequences=True,
                        go_backwards=True, kernel_initializer='he_normal',
                        name='decoder_gru_b', dropout=0.5)(res)
    decoder_merged = Lambda(concat_1)([decoder_gru, decoder_gru_b])

    y_pred = Dense(len(label_map) + 1, activation='softmax', kernel_initializer='he_normal',
    ...
    weight_file = 'text_recognize/weights%02d.h5' % (990)
...
def one_image(img, text_positions):  # [[[1,1], [2,2], [2,0], [1,0]]]
    '''
    识别单张图片
    :param img:
    :param text_position: [[左上，左下，右下，右上], []]
    :return:
    '''
    test_func = load_model("test")
    regular_text_images = []
    for point in text_positions:
        single_regular_area = regular_rectangle(img, point, 100, 100)
        regular_text_images.append(single_regular_area)

    for rti in regular_text_images:
        X_data = np.reshape(rti, [BATCH_SIZE, 100, 100, 3])

        # labels = np.zeros((BATCH_SIZE, 43))
        #
        # input_lengths = np.zeros([BATCH_SIZE, 1])
        # label_lengths = np.zeros([BATCH_SIZE, 1])
        #
...
def test(index):
    image_file_name = "/home/benke/veteranjy/data/images/tr_img_" + str(0) + str(index) + ".jpg"
    # image_file_name = "/home/benke/xuyichu/PSENet/tmp/tr_img_03001.jpg"
    img = cv2.imread(image_file_name)

    label_file_name = "/home/benke/veteranjy/data/labels/tr_img_" + str(0) + str(index) + ".txt"
    ...
    for line in lines:
        ...
        bottom_left = [int(elements[0]), int(elements[1])]
        bottom_right = [int(elements[2]), int(elements[3])]
        top_right = [int(elements[4]), int(elements[5])]
        top_left = [int(elements[6]), int(elements[7])]
