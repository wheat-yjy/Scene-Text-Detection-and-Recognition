...
import pickle
file = open('text_recognize/label_map_crnn_cej.pickle', 'rb+')
...
def load_model():
    ...
    inner = Conv2D(64, [3, 3], padding='same', activation='relu', kernel_initializer='he_normal', name='conv1')(input_data)
    ...
    gru = LSTM(256, return_sequences=True, kernel_initializer='he_normal', name='LSTM')(inner)
    gru_b = LSTM(256, return_sequences=True, go_backwards=True, kernel_initializer='he_normal', name='LSTM_B')(inner)
    gru_merged = Lambda(concat)([gru, gru_b])

    print(gru_merged.shape, "gru_merged")

    y_pred = Dense(len(label_map) + 3, activation='softmax', kernel_initializer='he_normal', name='dense_out')(gru_merged)
    ...
    sgd = SGD(lr=2e-4, momentum=0.9, nesterov=True, clipnorm=5)
    ...
    weight_file = './text_recognize/weights%02d.h5' % (11825)
...
def one_image(image, text_positions):
    ...
    for point in text_positions:
        ...
        plt.imshow(single_regular_area)
        plt.show()
...
def test(index):
    image_file_name = "/home/benke/veteranjy/data/images/tr_img_0" + str(index) + ".jpg"
    # image_file_name = "/home/benke/xuyichu/PSENet/tmp/tr_img_03001.jpg"
    img = cv2.imread(image_file_name, cv2.IMREAD_COLOR)
    # image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    label_file_name = "/home/benke/veteranjy/data/labels/tr_img_0" + str(index) + ".txt"
